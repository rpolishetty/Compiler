...
public class ParserCfg {
	...
	public ParserCfg(InputStream tl15In, OutputStream parseOut) throws Exception {
...
//		System.out.println(countNum);
	...
	private void Program(ArrayList program, ArrayList declarations,
		...
		for (int i = 0; i < (inputFileCopy.size());) {
			...
			if (inputFileCopy.get(i).equals("end")) {
				...
				ps.print("\\ljump Bexit \"]");

				ps.println();
				countNodeVal++;
				ps.print("n" + countNodeVal + "[label= \" Bexit \"]");

				ps.println();
				ps.println("n" + (countNodeVal - 1) + " -> n" + countNodeVal); // n0
																				// ->
																				// n1
				ps.println();
	...
	public ArrayList WhileStatement(ArrayList regCountValues, int regCount,
		...
		countNodeVal++;
		ps.print("n" + countNodeVal + "[label= \" B" + countNodeVal + ": "); // n3-----B3
		...
		ps.println();
		ps.println("n" + countNode1 + " -> n" + countNodeVal); // n3 -> n4 cbr
																// R26 -> B4, B5
		ps.println();

		ps.println();
		ps.println("n" + countNode1 + " -> n" + (countNodeVal + 1)); // n3 -> n5
																		// cbr
																		// R26
																		// ->
																		// B4,
		...
		ps.print("n" + countNodeVal + "[label= \" B" + countNodeVal + ": "); // B4
		...
		ps.print("\\ljump B" + (countNodeVal + 1) + "\"]"); // B5

		ps.println();
		ps.println("n" + countNodeVal + " -> n" + (countNodeVal + 1));
		ps.println();
	...
	public ArrayList IfStatement(ArrayList regCountValues, int regCount,
		...
		ps.print(" \\l jump B" + (countNodeVal + 2) + "\" ]");

		ps.println();
		ps.println("n" + count1 + " -> n" + countNodeVal); // n0 -> n1
		ps.println();

		ps.println();
		ps.println("n" + countNode1 + " -> n" + (countNodeVal + 2)); // n1 -> n3
		ps.println();
		...
		if (inputFileCopy.get(0).equals("else")) {
			...
			ps.print(" \\l jump B" + (countNodeVal + 1) + "\" ]");
			int countNode2 = countNodeVal;
			ps.println();

			ps.println("n" + count1 + " -> n" + countNodeVal); // n0 -> n2
			ps.println();

			ps.println();
			ps.println("n" + countNode2 + " -> n" + (countNodeVal + 1));
			ps.println();
...

	{
		...
		if (inputFileCopyVal1.equals("readint")) {
			...
			if (((htable.get(assignValues.get(0)) != null) && htable.get(
				...
				regCount++;
				ps.print("\\llloadI @" + assignValues.get(0) + " => R"
						+ regCount);
				regCount++;
				ps.print("\\lloadAO Rarp, R" + (regCount - 1) + " => R"
						+ regCount);
		...
		// executes if <expressionPrime> is <expression>
			...
			ps.print("\\li2i R" + regCount + " => R" + regCountLeft);
